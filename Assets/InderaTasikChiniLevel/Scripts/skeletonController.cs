...
public class skeletonController : MonoBehaviour, IDamageable
{
    ...
    public int health;
    [SerializeField] GameObject skeletonDeath; //death animation
    ...
    {
        FindObjectOfType<audioManager>().Play("skeletonHit"); //sound effect skeleton hit
    ...
    public void Die()
    {
        FindObjectOfType<audioManager>().Play("skeletonDie"); //sound effect skeleton dies
        
        GameObject instance = Instantiate(skeletonDeath, transform.position, transform.rotation);
    ...
    void Update()
    {
        ...
        animator.SetInteger("skellyState", 0); // idle skelly
        ...
        else
        {
            ...
            else if (distanceFromPlayer < lineOfSight)
            {
                if (stunTime >= 1f)
                {
                    ...
                    animator.SetInteger("skellyState", 1); // walk skelly
        ...
        if (playerInRange() == true)
        {
            ...
            {
                //Debug.Log("Attacking");
                FindObjectOfType<audioManager>().Play("skeletonSlash");
                cooldownTimer = 0;
                animator.SetInteger("skellyState", 2); // skelly attack
                attackTimer = 0;
